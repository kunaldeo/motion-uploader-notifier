...
'''
@author: Kunal Deo
Original code by Jeremy Blythe http://jeremyblythe.blogspot.com
'''
...
class MotionUploader:
    def __init__(self, config_file_path):
        ...
        self.pushover_enabled = config.getboolean('pushover', 'enable')
        self.app_token = config.get('pushover', 'app_token')
        self.user_key = config.get('pushover', 'user_key')
        self.msg_title = config.get('pushover', 'msg_title')
        self.msg_body = config.get('pushover', 'msg_body')
    ...
    def _send_pushover_notification(self):
        con = httplib.HTTPSConnection("api.pushover.net:443")
        con.request("POST", "/1/messages.json",
            urllib.urlencode({
...
if __name__ == '__main__':         
    try:
        if len(sys.argv) < 3:
            exit('Motion Uploader - uploads videos to Google Drive, Email and Send Pushover Notification\n Usage: uploader.py {config-file-path} {video-file-path}')
        cfg_path = sys.argv[1]
        vid_path = sys.argv[2]    
